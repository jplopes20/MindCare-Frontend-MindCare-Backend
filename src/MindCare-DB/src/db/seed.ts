...
export async function seedIfEmpty() {
  try {
    ...
    console.log('[seed] Hash de senha atualizado para todos os usuários seed')

    const insertedUsers = await db.select().from(users).where(inArray(users.email, SEED_EMAILS))
